...
def render_stardew_operator(view: OperatorView) -> str:
    ...
    failure = ""
    if view.failure:
        failure = f'<section class="failure" role="alert" data-testid="failure"><h2>Stopped safely</h2><p>First unmet requirement: {html.escape(view.failure.first_unmet_requirement)}</p><p>{html.escape(view.failure.safe_recovery)}</p></section>'
    save_label = "No copied save"
    if view.save:
        save_label = f"Disposable copy · {view.save.disposable_tree_sha256[:12]}"
    tell_text = "Tell is waiting for a fresh exact observation."
    ...
    evidence_text = ", ".join(view.observation_evidence) or "No current retained screenshot evidence"
...
<title>Stardew Operator</title><style>
...
<section><h2>Do / Takeover</h2><p>Scope: <strong data-testid="do-scope">{html.escape(do_scope)}</strong> · Expiry: {html.escape(do_expiry)}</p><p>Neutralization: <strong data-testid="neutralization-status">{html.escape(view.neutralization_status)}</strong> · Handback: <strong data-testid="handback-status">{html.escape(view.handback_status)}</strong></p><div class="actions"><button{do_disabled}>Authorize one watering task</button><button{reclaim_disabled}>Take control now</button></div><p class="muted">Controls remain disabled until their exact live preconditions and fresh authorization exist.</p></section>
<section><h2>Task boundary and reset</h2><p>Water the exact crops observed at task start, then return to the farmhouse entrance. Stop on any unknown, occlusion, save/process mismatch, ownership ambiguity, or protected action.</p><p>Final position: <strong data-testid="final-position">{html.escape(position)}</strong></p><p>Evidence: {html.escape(view.evidence_status)}</p><p>Reset: <strong data-testid="reset-status">{html.escape(view.reset_status)}</strong>. Reset creates a fresh attempt-owned copy and preserves the prior attempt.</p><button disabled>Reset disposable copy</button></section>
<section><h2>Safety</h2><p data-testid="protected-action-refusal">{html.escape(protected)}</p><p>First failure and safe recovery remain visible above; every failure neutralizes input before handback.</p></section>
</main></body></html>"""
